...
class JaniUnfolder():
    ...
    def construct_edges(self, edge, design_space, expression_variables):
        ...
        relevant_guard = {hole_index for hole_index in design_space.hole_indices if
                          expression_variables[hole_index] in variables}

        # relevant holes in probabilities
        variables = set().union(*[d.probability.get_variables() for d in edge.destinations])
        relevant_probs = {hole_index for hole_index in design_space.hole_indices if
                          expression_variables[hole_index] in variables}
        ...
        relevant_updates = {hole_index for hole_index in design_space.hole_indices if
                            expression_variables[hole_index] in variables}
