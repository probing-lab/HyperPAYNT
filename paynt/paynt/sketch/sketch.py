...
class Sketch:
    ...
    def __init__(self, sketch_path, properties_path, constant_str):
        ...
        if self.is_explicit:
            ...
            if len(hole_definitions) == 0:
                ...
                self.prism, self.hole_expressions, self.design_space = Sketch.parse_holes(self.prism, expression_parser,
                                                                                          hole_definitions)
    ...
    def parse_holes(cls, prism, expression_parser, hole_definitions):
        ...
        for hole_index, hole in enumerate(holes):
            if hole.size == 1:
                trivial_holes_definitions[prism.get_constant(hole.name).expression_variable] = \
                hole_expressions[hole_index][0]
            else:
